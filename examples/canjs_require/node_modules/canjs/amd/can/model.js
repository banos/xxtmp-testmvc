...
define(["can/util/library", "can/map", "can/list"], function (can) {
	...
	can.Model = can.Map({
			 ...
			 *
			 *     {tobuy: {name: "dishes", id: 5}}
			 *
			 * Overwrite `model` like:
			 *
			 *     Task = can.Model.extend({
			 *       model : function(data){
			 *         return can.Model.model.call(this,data.tobuy);
			 *       }
			 *     },{});
			 */
		 ...
		 */
		{
			 ...
			 *     var tobuy = new Tobuy({name: "dishes"})
			 *
			 *     // listen when the instance is created
			 *     tobuy.bind("created", function(ev){
			 *      this //-> tobuy
			 *     })
			 *
			 *     // save it on the server
			 *     tobuy.save(function(tobuy){
			 *      console.log("tobuy", tobuy, "created")
			 *     });
			 *
			 ...
			 *     Tobuy.findOne({id: 5},function(tobuy){
			 *
			 *       // listen when the instance is updated
			 *       tobuy.bind("updated", function(ev){
			 *          this //-> tobuy
			 *       })
			 *
			 *       // update the instance's property
			 *       tobuy.attr("complete", true)
			 *
			 *       // save it on the server
			 *       tobuy.save(function(tobuy){
			 *          console.log("tobuy", tobuy, "updated")
			 *       });
			 *
			 *     });
			 *
	...
	can.each({
		 ...
		 *     },{})
		 *
		 * The following Tobuy model will never request the same list of tobuy's twice:
		 *
		 *     Tobuy = CachedModel({
		 *       findAll: "/tobuys"
		 *     },{})
		 ...
		 *     },{})
		 *
		 * The following Tobuy model will never request the same tobuy twice:
		 *
		 *     Tobuy = CachedModel({
		 *       findOne: "/tobuys/{id}"
		 *     },{})
