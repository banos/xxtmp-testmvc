...
var DOMProperty = {
   ...
   *
   * TOBUY: Is it better to grab all the possible properties when creating an
...
var EnterLeaveEventPlugin = {
  ...
  extractEvents: function(
    ...
    if (topLevelTarget.window === topLevelTarget) {
      ...
      // TOBUY: Figure out why `ownerDocument` is sometimes undefined in IE8.
...
var ReactChildReconciler = {
  ...
  updateChildren: function(
    ...
    // moved.
    // TOBUY: If nothing has changed, return the prevChildren object so that we
 ...
 * leaf child.
 *
 * TOBUY: This may likely break any calls to `ReactChildren.map` that were
...
var ReactClassMixin = {
  ...
  /**
   * TOBUY: This will be deprecated because state should always keep a consistent
 ...
 * the browser context. TOBUY: These callers should be refactored to avoid the
...
var ReactCompositeComponentMixin = {
  ...
  unmountComponent: function() {
    ...
    // TOBUY: inst.props = null;
    // TOBUY: inst.state = null;
    // TOBUY: inst.context = null;
  },
  ...
  _checkPropTypes: function(propTypes, props, location) {
    // TOBUY: Stop validating prop types here and only use the element
    // validation.
   ...
   * TOBUY: Remove this check when owner-context is removed
   */
  ...
  _renderValidatedComponent: function(childContext) {
    ...
    ("production" !== "development" ? invariant(
      // TOBUY: An `isValidNode` function would probably be more appropriate
      renderedComponent === null || renderedComponent === false ||
      ReactElement.isValidElement(renderedComponent),
      '%s.render(): A valid ReactComponent must be returned. You may have ' +
        'returned undefined, an array or some other invalid object.',
      this.getName() || 'ReactCompositeComponent'
    ) : invariant(// TOBUY: An `isValidNode` function would probably be more appropriate
    renderedComponent === null || renderedComponent === false ||
    ReactElement.isValidElement(renderedComponent)));
    return renderedComponent;
  },
...
ReactDOMComponent.Mixin = {
   ...
   *
   * TOBUY: Benchmark whether checking for changed values in memory actually
   *       improves performance (especially statically positioned elements).
   * TOBUY: Benchmark the effects of putting this at the top since 99% of props
   *       do not change for a given reconciliation.
   * TOBUY: Benchmark areas that can be improved with caching.
   *
   * @private
...
var ReactDOMForm = ReactClass.createClass({
  ...
  render: function() {
    // TOBUY: Instead of using `ReactDOM` directly, we should use JSX. However,
...
var ReactDOMOption = ReactClass.createClass({
  ...
  componentWillMount: function() {
    // TOBUY (yungsters): Remove support for `selected` in <option>.
    if ("production" !== "development") {
      ("production" !== "development" ? warning(
 ...
 *
 * TOBUY: Investigate representing React components in the DOM with text nodes.
 *
...
assign(ReactDOMTextComponent.prototype, {
  ...
  construct: function(text) {
    // TOBUY: This is really a ReactText (ReactNode), not a ReactElement
  ...
  receiveComponent: function(nextText, transaction) {
    if (nextText !== this._currentElement) {
      ...
      if (nextStringText !== this._stringText) {
        // TOBUY: Save this as pending props and use performUpdateIfNecessary
...
var ReactDOMTextarea = ReactClass.createClass({
  ...
  getInitialState: function() {
    var defaultValue = this.props.defaultValue;
    // TOBUY (yungsters): Remove support for children content in <textarea>.
...
var ReactDefaultPerf = {
  ...
  printExclusive: function(measurements) {
    ...
    // TOBUY: ReactDefaultPerfAnalysis.getTotalTime() does not return the correct
    // number.
  },
  ...
  _recordWrite: function(id, fnName, totalTime, args) {
    // TOBUY: totalTime isn't that useful since it doesn't count paints/reflows
    var writes =
      ReactDefaultPerf
...
function getTotalTime(measurements) {
  // TOBUY: return number of DOM ops? could be misleading.
  // TOBUY: measure dropped frames after reconcile?
  // TOBUY: log total time of each reconcile and the top-level component
...
var ReactElement = function(type, key, ref, owner, context, props) {
  ...
  // TOBUY: Deprecate withContext, and then the context becomes accessible
  // through the owner.
  this._context = context;
...
ReactElement.createFactory = function(type) {
  ...
  // Legacy hook TOBUY: Warn if this is accessed
  factory.type = type;
  return factory;
};
...
ReactElement.isValidElement = function(object) {
  ...
  // TOBUY: We could possibly warn here.
  // }
  return isElement;
};
...
function validatePropTypes(element) {
  ...
  // TOBUY: Validating a string's propTypes is not decoupled from the
...
var ReactElementValidator = {
  ...
  createElement: function(type, props, children) {
    ...
    // TOBUY: Drop this when these are no longer allowed as the type argument.
    if (element == null) {
      return element;
    }
...
ReactEmptyComponentType.prototype.componentDidMount = function() {
  var internalInstance = ReactInstanceMap.get(this);
  // TOBUY: Make sure we run these methods in the correct order, we shouldn't
...
ReactEmptyComponentType.prototype.componentWillUnmount = function() {
  var internalInstance = ReactInstanceMap.get(this);
  // TOBUY: Get rid of this check. See TOBUY in componentDidMount.
  if (!internalInstance) {
    return;
  }
...
function findParent(node) {
  // TOBUY: It may be a good idea to cache this to prevent unnecessary DOM
...
// TOBUY: Replace this with ES6: var ReactInstanceMap = new Map();
  ...
  Mixin: {
    ...
    updateTextContent: function(nextContent) {
      ...
      try {
        ...
        // TOBUY: The setTextContent operation should be enough
...
var Mixin = {
   ...
   *   TOBUY: convert to array<TransactionWrapper>
   */
  getTransactionWrappers: function() {
    return TRANSACTION_WRAPPERS;
  },
...
ReactRef.shouldUpdateRefs = function(prevElement, nextElement) {
  ...
  // TOBUY: Should this even be possible? The owner cannot change because
...
/**
 * Tobuy: Support the entire DOM.scry query syntax. For now, these simple
...
var ReactTestUtils = {
  ...
  isDOMComponent: function(inst) {
    // TOBUY: Fix this heuristic. It's just here because composites can currently
...
var ReactTransitionGroup = React.createClass({
  ...
  render: function() {
    // TOBUY: we could get rid of the need for the wrapper node
...
var ReactUpdateQueue = {
  ...
  enqueueCallback: function(publicInstance, callback) {
    ...
    // TOBUY: The callback here is ignored when setState is called from
   ...
   * properties which is confusing. TOBUY: Expose pendingState or don't use it
   * during the merge.
   *
   * @param {ReactClass} publicInstance The instance that should rerender.
...
if (ExecutionEnvironment.canUseDOM) {
  ...
  if (testElement.innerHTML === '') {
    setInnerHTML = function(node, html) {
      ...
      // TOBUY: What to do on a detached node?
      if (node.parentNode) {
...
/**
 * TOBUY: Test that a single child and an array with one item have the same key
 * pattern.
 */
